...
public class Bag {
    ...
    public void addCash(String item, long quantity) {
        ...
        if (hasFreeCapacity(quantity) && totalGems >= totalCash + quantity) {
            if (this.cash.containsKey(item)) {
                ...
                this.cash.putIfAbsent(item, quantity);
            }
    ...
    public void addGems(String item, long quantity) {
        ...
        if (hasFreeCapacity(quantity) && totalGems + quantity <= this.gold) {
            if (this.gems.containsKey(item)) {
                ...
                this.gems.putIfAbsent(item, quantity);
            }
            this.currentTotalQuantity += quantity;
        }
    }
    ...
    public void printContent() {
        ...
        TreeMap<String, Long> mapToSort = new TreeMap<>(Comparator.reverseOrder());
        if (this.gold != 0) {
            mapToSort.put("Gold", this.gold);
        } if (this.gems != null) {
            mapToSort.put("Gem", getTotalGems());
        } if (this.cash != null) {
            mapToSort.put("Cash", getTotalCash());
        }

        for (Map.Entry<String, Long> pair : mapToSort.entrySet()) {
            if (pair.getKey().equals("Gold")) {
                sb.append(String.format("<Gold> $%d", this.gold)).append(System.lineSeparator());
                sb.append(String.format("##Gold - %d", this.gold)).append(System.lineSeparator());
            } else if (pair.getKey().equals("Cash") && !this.cash.isEmpty()) {
            ...
            } else if (pair.getKey().equals("Gem") && !this.gems.isEmpty()) {
